...
CREATE TABLE public.resume_analyses (
  ...
  keyword_match_percentage INTEGER DEFAULT 0,
  ...
  industry_suggestions TEXT[] DEFAULT '{}',
...
CREATE POLICY "Users can create their own analyses" ON public.resume_analyses FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own analyses" ON public.resume_analyses FOR UPDATE USING (auth.uid() = user_id);
...
CREATE TRIGGER update_analyses_updated_at BEFORE UPDATE ON public.resume_analyses FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();